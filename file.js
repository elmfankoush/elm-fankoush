...
const EPISODES = {
  ...
  10: { suffix:'10',  title:'علم الأجسام الطائرة المجهولة', desc:'بين الشهادات الحقيقية والتفسيرات العلمية',               endingDesc:'الصورة الكاملة — ماذا يقول العلم عن الأجسام الطائرة المجهولة؟',  overlayQ:'الأجسام الطائرة المجهولة…<br><span style="color:var(--elm)">علم</span> ولا <span style="color:var(--fan)">فنكوش</span>؟' },
...
// ══ choosePath: يصوت ويعرض نتائج لحظية ثم يكمل ══
function choosePath(path){
  if(window.castChoiceVoteAndShow){
    window.castChoiceVoteAndShow(currentEp, path, () => {
      hideOverlay();
...
// ══ Ending Vote Overlay (مربعين في آخر الحلقة) ══
function showEndingVoteOverlay(){
  olay.classList.remove('visible');
  overlayBtns.innerHTML = '';
  overlayQ.innerHTML  = 'أنت في النهاية اخترت إيه؟';
  overlayHint.textContent = 'صوّت وشوف رأي المشاهدين';

  if(window.setupEndingVote){
    window.setupEndingVote(currentEp, overlayBtns, overlayHint);
  } else {
    // fallback لو Firebase ما حملتش
    overlayBtns.innerHTML = `
      <div class="overlay-btns-choice">
        <button class="ov-btn elm" onclick="event.stopPropagation()">🔬 علم</button>
        <button class="ov-btn fan" onclick="event.stopPropagation()">🔮 فنكوش</button>
      </div>`;
  }
  requestAnimationFrame(() => requestAnimationFrame(() => olay.classList.add('visible')));
}
...
vid.addEventListener('ended', () => {
  ...
  if(f.type === 'auto'){
    ...
    showRestartBtn();
    // نظهر مربعي التصويت بعد 800ms
    setTimeout(() => showEndingVoteOverlay(), 800);
  } else {
    showOverlay(curSeg);
  }
});
